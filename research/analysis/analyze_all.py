...
# Graph internal name -> display label for x-axis tick labels
GRAPH_DISPLAY_NAMES = {
...
def add_graph_metadata(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['graph_label'] = df['graph_base'].map(
        lambda x: GRAPH_DISPLAY_NAMES.get(x, x)
    )
...
# Canonical name mapping: lowercase -> canonical (title-case key used in IMPL_ORDER)
_IMPL_CANONICAL = {
...
def _canonical_impl_key(name: str) -> str:
    """Convert any implementation name variant to its canonical capitalized key."""
    return _IMPL_CANONICAL.get(name.lower().replace('-', '_'), name)


def normalize_impl_names(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize implementation names to canonical capitalized form for table generation."""
    df = df.copy()
    df['Implementation'] = df['Implementation'].map(
        lambda x: _canonical_impl_key(x) if isinstance(x, str) else x
    )
    return df
...
def plot_execution_time(df: pd.DataFrame, out_dir: Path):
    ...
    for impl in get_impl_order(df):
        ...
        # Exclude <= 0 values before log-scale
        sub = sub[sub['Time_sec'] > 0]
        if sub.empty:
            continue
        xs = list(range(len(sub)))
        ys = sub['Time_sec'].values
        xlabels = sub['graph_label'].tolist()
    ...
    all_labels = [GRAPH_DISPLAY_NAMES.get(g, g) for g in graphs_sorted]
    ...
    fig.tight_layout()
    out_path = out_dir / 'execution_time_comparison.pdf'
...
def plot_speedup(df: pd.DataFrame, out_dir: Path, baseline_impl: str = 'sequential'):
    ...
    ax.axhline(1.0, color='gray', linestyle='--', linewidth=1, label=f'{baseline_label} (baseline)')
    ax.set_xlabel('Number of Vertices')
    ax.set_ylabel('Speedup (x)')
    # Choose a short name for the file suffix
    impl_short = baseline_impl.replace(' ', '_')
    if 'sequential' in baseline_impl.lower():
        title = 'Speedup over Sequential (Single-thread CPU)'
        file_suffix = 'Sequential'
    elif 'omp' in baseline_impl.lower() or 'openmp' in baseline_impl.lower():
        title = 'Speedup over OpenMP (72-core Parallel)'
        file_suffix = 'OpenMP'
    else:
        title = f'Speedup over {baseline_label}'
        file_suffix = baseline_impl
    ax.set_title(title)
    n_impls = len([i for i in get_impl_order(df) if i != baseline_impl])
    if n_impls >= 4:
        ax.legend(bbox_to_anchor=(1.01, 1), loc='upper left', borderaxespad=0)
    else:
        ax.legend(loc='upper left')
    ...
    fig.tight_layout()
    out_path = out_dir / f'speedup_vs_{file_suffix}.pdf'
    fig.savefig(out_path, bbox_inches='tight')
    fig.savefig(out_path.with_suffix('.png'), dpi=FIGURE_DPI, bbox_inches='tight')
    print(f"[OK] {out_path}")
    plt.close(fig)
...
def plot_phase2_comparison(df: pd.DataFrame, out_dir: Path):
    """Phase2: execution time comparison for gpu / gpu_stream / gpu_managed / gpu_opt (grouped bar chart)."""
    if df.empty:
        print("[SKIP] Phase2 graph: no data")
        return

    df = add_graph_metadata(df)
    impls = ['GPU', 'GPU_Stream', 'GPU_Managed', 'GPU_Opt',
             'gpu', 'gpu_stream', 'gpu_managed', 'gpu_opt']
    ...
    for i, impl in enumerate(impls_present):
        ...
        times = [sub[sub['graph_base'] == g]['Time_sec'].values[0]
                 if g in sub['graph_base'].values else np.nan
    ...
    labels = [GRAPH_DISPLAY_NAMES.get(g, g) for g in graphs_sorted]
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=35, ha='right')
    ax.set_yscale('log')
    ax.set_xlabel('Number of Vertices')
    ax.set_ylabel('Execution Time (seconds)')
    ax.set_title('GH200 Memory Strategy Comparison\n(HBM3 vs. Unified Memory)')
    if n_impls >= 4:
        ax.legend(bbox_to_anchor=(1.01, 1), loc='upper left', borderaxespad=0)
    else:
        ax.legend()
...
def plot_bandwidth(df: pd.DataFrame, out_dir: Path):
    ...
    ax.set_title(f'GH200 Memory Bandwidth: HBM3 vs. NVLink-C2C (buffer={max_size:.1f} GB)')
    ax.legend()
    ax.grid(True, axis='y', alpha=0.3)
...
def plot_batchsize_sweep(df: pd.DataFrame, out_dir: Path):
    ...
    for ax, metric, ylabel in zip(
        ...
        for g in graphs:
            ...
            label = GRAPH_DISPLAY_NAMES.get(g, g)
...
def make_exec_time_table(df: pd.DataFrame, tables_dir: Path):
    ...
    df = add_graph_metadata(normalize_impl_names(df))
...
def make_speedup_table(df: pd.DataFrame, tables_dir: Path,
    ...
    df = add_graph_metadata(normalize_impl_names(df))
    # Normalize baseline_impl name too
    baseline_impl_norm = _canonical_impl_key(baseline_impl)
    base_df = df[df['Implementation'] == baseline_impl_norm][['graph_base', 'Time_sec']].rename(
        columns={'Time_sec': 'base_time'}
    )
    if base_df.empty:
        print(f"[SKIP] Speedup table: no baseline data for '{baseline_impl_norm}'")
        return
    ...
    tsv_path = tables_dir / f'speedup_table_vs_{baseline_impl_norm}.tsv'
    pivot.to_csv(tsv_path, sep='\t', float_format='%.2f')
    print(f"[OK] {tsv_path}")

    latex_path = tables_dir / f'speedup_table_vs_{baseline_impl_norm}.tex'
    col_labels = [IMPL_LABELS.get(c, c) for c in pivot.columns]
    baseline_label = IMPL_LABELS.get(baseline_impl_norm, baseline_impl_norm)
    with open(latex_path, 'w') as f:
        f.write("\\begin{table}[h]\n")
        f.write("\\centering\n")
        f.write(f"\\caption{{Speedup over {baseline_label}}}\n")
        f.write(f"\\label{{tab:speedup_vs_{baseline_impl_norm}}}\n")
        f.write(f"\\begin{{tabular}}{{l{'r' * len(pivot.columns)}}}\n")
        f.write("\\hline\n")
        f.write("Graph & " + " & ".join(col_labels) + " \\\\\n")
        f.write("\\hline\n")
...
def plot_ablation(df: pd.DataFrame, out_dir: Path):
    """Ablation study: GTEPS comparison for GPU -> GPU_Stream -> GPU_Managed -> GPU_ReadMostly -> GPU_Opt.

    Visualizes the independent contribution of Method 1 (ReadMostly) and Method 2 (2-stream / double-buffer).
    GPU_Stream is Stage 1: HBM3 + double-buffer (no UVM).
    """
    if df.empty:
    ...
    # Warn if GPU_Stream is missing
    if 'GPU_Stream' not in impls_present:
        print("[WARN] Ablation graph: GPU_Stream (Stage 1) data not found — skipping that series")
    ...
    if ref_impl in impls_present:
        ...
        ax2.legend(bbox_to_anchor=(1.01, 1), loc='upper left', borderaxespad=0, fontsize=8)
...
def plot_threshold_sensitivity(path: Path, out_dir: Path):
    """Topology placement threshold sensitivity figure.

    Data source: data/threshold_sensitivity.tsv
    Columns: Graph  Threshold  Time_sec
    """
    if not path.exists():
        print(f"[SKIP] Threshold sensitivity: {path} not found")
        return
    ...
    try:
        ...
        print(f"[SKIP] Threshold sensitivity: failed to load {path}: {e}")
        return

    if df.empty or 'Threshold' not in df.columns or 'Time_sec' not in df.columns:
        print("[SKIP] Threshold sensitivity: missing required columns (Threshold, Time_sec)")
        return
    ...
    for g in sorted(graphs):
        sub = df[df['Graph'].apply(lambda x: Path(x).name) == g].sort_values('Threshold')
        if sub.empty:
            continue
        label = GRAPH_DISPLAY_NAMES.get(g, g)
        ax.plot(sub['Threshold'], sub['Time_sec'], marker='o', label=label, linewidth=1.8, markersize=6)

    ax.axvline(default_threshold, color='red', linestyle='--', linewidth=1.2,
               label=f'Default ({default_threshold})')
    ax.text(default_threshold + 0.005, ax.get_ylim()[1] * 0.95 if ax.get_ylim()[1] > 0 else 1,
            f'Default ({default_threshold})', color='red', fontsize=9, va='top')

    ax.set_xscale('log')
    ax.set_xlabel('Topology Placement Threshold')
    ax.set_ylabel('Execution Time (seconds)')
    ax.set_title('Topology Placement Threshold Sensitivity (--topo-threshold)')
    ax.legend(bbox_to_anchor=(1.01, 1), loc='upper left', borderaxespad=0)
    ax.grid(True, which='both', alpha=0.3)

    fig.tight_layout()
    out_path = out_dir / 'threshold_sensitivity.pdf'
    fig.savefig(out_path, bbox_inches='tight')
    fig.savefig(out_path.with_suffix('.png'), dpi=FIGURE_DPI, bbox_inches='tight')
    print(f"[OK] {out_path}")
    plt.close(fig)
...
def main():
    ...
    # Try repo-relative path first, then result_dir relative
    threshold_path = Path(__file__).parent.parent.parent / 'data' / 'threshold_sensitivity.tsv'
    if not threshold_path.exists():
        threshold_path = result_dir / 'threshold_sensitivity.tsv'
