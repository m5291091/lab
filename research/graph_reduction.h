...
// ----------------------------------------------------------
// ReducedGraph 上で Brandes 法 (逐次) を実行し、BC 値を返す
...
// ============================================================
//
// Degree-1 頂点の除去は残存頂点間の最短経路を変えないため、
// 縮約グラフの Brandes 結果 + BFS ベースの補正で正確な復元が可能。
//
// Degree-2 チェーン圧縮と Twin 統合は辺構造を変更するため、
// 正確な復元には重み付き Brandes が必要。
// 本パイプラインでは Degree-1 peeling のみ BC 前処理に使用する。
// ----------------------------------------------------------
...
// ----------------------------------------------------------
// End-to-End パイプライン:
//   Step 1 (Degree-1 Peeling) → BC 計算 → Step 5 (BFS 復元)
//   Steps 2, 3 は参考情報として縮約率を出力
