...
static inline cudaMemLocation make_device_location(int device_id) {
    cudaMemLocation loc{};
...
static inline cudaMemLocation make_host_location() {
    cudaMemLocation loc{};
...
static inline cudaError_t cuda_mem_prefetch_async_compat(const void *ptr, size_t size,
                                                          int device_id, cudaStream_t stream = 0) {
#if CUDART_VERSION >= 13000
    return cudaMemPrefetchAsync(ptr, size, make_device_location(device_id), 0u, stream);
#else
    return cudaMemPrefetchAsync(ptr, size, device_id, stream);
#endif
}

static inline cudaError_t cuda_mem_prefetch_host_async_compat(const void *ptr, size_t size,
                                                               cudaStream_t stream = 0) {
#if CUDART_VERSION >= 13000
    return cudaMemPrefetchAsync(ptr, size, make_host_location(), 0u, stream);
#else
    return cudaMemPrefetchAsync(ptr, size, cudaCpuDeviceId, stream);
#endif
}

static inline cudaError_t cuda_mem_advise_device_compat(const void *ptr, size_t size,
                                                         cudaMemoryAdvise advice, int device_id) {
#if CUDART_VERSION >= 13000
    return cudaMemAdvise(ptr, size, advice, make_device_location(device_id));
#else
    return cudaMemAdvise(ptr, size, advice, device_id);
#endif
}

static inline cudaError_t cuda_mem_advise_host_compat(const void *ptr, size_t size,
                                                       cudaMemoryAdvise advice) {
#if CUDART_VERSION >= 13000
    return cudaMemAdvise(ptr, size, advice, make_host_location());
#else
...
static double measure_c2c_prefetch_bandwidth(size_t size, int gpu_id, int reps=5) {
    ...
    CUDA_ERR_CHK(cuda_mem_advise_host_compat(ptr, size, cudaMemAdviseSetPreferredLocation));
    CUDA_ERR_CHK(cuda_mem_advise_device_compat(ptr, size, cudaMemAdviseSetAccessedBy, gpu_id));
    ...
    // ワームアップ
    for (int i = 0; i < 3; i++) {
        CUDA_ERR_CHK(cuda_mem_prefetch_async_compat(ptr, size, gpu_id, 0));
        CUDA_ERR_CHK(cuda_mem_prefetch_host_async_compat(ptr, size, 0));
        CUDA_ERR_CHK(cudaDeviceSynchronize());
    }
