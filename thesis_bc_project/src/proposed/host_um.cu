        ...
        int n_nodes, int edge_size, int gpu_id, double h2d_time = 0.0)
{
    ...
    // ---- 診断スイッチ (既定 OFF) --------------------------------------------
    // 未指定 or 0 のとき checkpoint ac2b409 と数値・コードパス完全一致。
    // 正確性原因の 1 因子切り分け専用 (性能測定ではない)。GPU_Opt (host_um) のみに追加。
    //   BC_DIAG_FORCE_FULL_RESET=1 : visited-only reset gating を使わず常に full memset。
    //   BC_DIAG_FORCE_NS_EFF_ONE=1 : in-capacity でも NS_eff=1 (単一 stream/occupancy 診断)。
    //     ※ requested/effective batch, SUB_BATCH, num_subs, oversubscribed, UM 割当方式,
    //        BFS/backward kernel, tpb, max_depth_estimate, 許容値は不変。UM oversubscription
    //        (prefetch/evict stress 経路) ではなく stream/occupancy の診断である。
    auto diag_flag = [](const char *name) { const char *e = getenv(name); return e && atoi(e) != 0; };
    const bool diag_force_full_reset = diag_flag("BC_DIAG_FORCE_FULL_RESET");
    const bool diag_force_ns_eff_one = diag_flag("BC_DIAG_FORCE_NS_EFF_ONE");
    const bool diag_mode = diag_force_full_reset || diag_force_ns_eff_one;
    fprintf(stderr, "  > [Diag] force_full_reset=%s\n", diag_force_full_reset ? "true" : "false");
    fprintf(stderr, "  > [Diag] force_ns_eff_one=%s\n", diag_force_ns_eff_one ? "true" : "false");
    fprintf(stderr, "  > [Diag] diagnostic_mode=%s\n",  diag_mode ? "true" : "false");
    ...
    const int  NS_eff = (oversubscribed || diag_force_ns_eff_one) ? 1 : NS;
    ...
    *d_overflow = 0;
    ...
    // ============================================================
    bool buf_used[NS] = {};
    // 実行経路カウンタ (診断用): 各 sub-batch で実際に選択された reset 分岐だけを加算。
    // GPU 計算式・reset kernel・memset 処理自体は変更しない (host 側の分岐計数のみ)。
    int full_memset_calls = 0;
    int visited_reset_calls = 0;
    for (int s_start = 0; s_start < n_nodes; s_start += BATCH_PER_STREAM) {
        ...
        for (int sub_off = 0; sub_off < curr_batch; sub_off += SUB_BATCH) {
            ...
            //   BC_DIAG_FORCE_FULL_RESET=1 のときは gating を無効化し常に full memset (診断)。
            nvtxRangePushA("Memset_or_reset_subbatch_opt");
            if (!diag_force_full_reset
    ...
    fprintf(stderr, "  > [Diag] full_memset_calls=%d\n",  full_memset_calls);
    fprintf(stderr, "  > [Diag] visited_reset_calls=%d\n", visited_reset_calls);
#ifdef DEBUG_BRANDES
    fprintf(stderr,
