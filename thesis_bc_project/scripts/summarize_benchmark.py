...
def main():
    ...
    # raw の随伴ファイル(phase_timing.log / max_bc.tsv)は results.tsv と同じ入力ディレクトリを
    # 優先探索し(raw_data/ 分離対応)、無ければ output_dir にフォールバック(旧 in-place 互換)。
    input_dir = os.path.dirname(os.path.abspath(tsv_path))

    # --- Phase timing ---
    phase_log_path = os.path.join(input_dir, "phase_timing.log")
    ...
    # --- Max BC (correctness) ---
    maxbc_path = os.path.join(input_dir, "max_bc.tsv")
    if not os.path.exists(maxbc_path):
