...
def run_wilcoxon(trials_opt, trials_pure):
    ...
    try:
        ...
        if p > 0.05:
            # p>0.05 は「有意差なし」= 同等性の証明ではない。検出力不足の可能性を明示する。
            return f"p={p:.3f} (n={n} では有意差を検出できず; 同等性の証明ではない)"
        else:
            return f"p={p:.3f} (有意差あり)"
...
def plot_phase_breakdown(avg_phases, out_pdf, totals=None):
    if not HAS_LIBS:
    ...
    for ax, g in zip(axes, graphs):
        ...
        for m in methods:
            if (m, g) in avg_phases:
                p = avg_phases[(m, g)]
                measured = p['h2d'] + p['bfs'] + p['bwd'] + p['d2h']
                # 未計測 (Other): end-to-end 総時間 - 計測済み成分。
                # 現行 UM ログは BFS/Backward (+Prefetch) のみを持ち、H2D/D2H 転送・init・
                # 結果回収などは個別計測されていない。総時間との差を Other として明示する。
                total = totals.get((m, g)) if totals else None
                other = max(0.0, total - measured) if total else 0.0
                data.append([p['h2d'], p['bfs'], p['bwd'], p['d2h'], other])
                labels.append(m)
        ...
        # UM 版では h2d=Prefetch, d2h=0。BFS/Backward が主計測成分。
        # 図ラベルは英字 (CJK フォント非依存で確実に描画するため)。
        names = ['H2D/Prefetch (measured)', 'BFS (kernel)', 'Backward (kernel)',
                 'D2H (measured)', 'Other (unmeasured: H2D/D2H xfer, init)']
        
        for i in range(5):
            ax.barh(labels, data[:, i], left=bottoms, color=colors[i], label=names[i])
            bottoms += data[:, i]
            
        ax.set_title(f"Phase Breakdown (BFS/Backward measured components + unmeasured Other): {g}")
...
if __name__ == "__main__":
    ...
    if args.phases:
        ...
        # end-to-end 総時間 (mean) を渡し、未計測分を Other として図に明示する。
        totals = {k: (sum(v) / len(v)) for k, v in raw_trials.items() if v}
