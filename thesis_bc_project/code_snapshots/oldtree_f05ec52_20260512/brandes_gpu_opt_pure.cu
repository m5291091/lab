...
// [最適化3.5] shared-frontier カーネル定数 (_pure.cu)
...
__device__ void accumulate_dependencies_tpv_opt_pure(
        int *R, int *C, int *d_d, double *d_sigma, double *d_delta,
        int *d_S, int *d_S_ends, int batch_idx, int n_nodes, int max_depth_estimate, int &depth)
{
...
__global__ void reset_visited_batch_kernel_pure(
...
__global__ void brandes_bfs_kernel_shared_frontier_pure(
        int *R, int *C, int n_nodes,
        int max_depth_estimate, int *d_overflow,
        int *d_d, double *d_sigma, int *d_Q_curr, int *d_Q_next,
...
__global__ void brandes_back_kernel_tpv_opt_pure(
        int *R, int *C, double *CB, int n_nodes, int max_depth_estimate,
        int *d_d, double *d_sigma, double *d_delta,
        int *d_S, int *d_S_ends, const int *d_depth, int s_start)
{
    ...
    accumulate_dependencies_tpv_opt_pure(R, C, d_d, d_sigma, d_delta,
                                         d_S, d_S_ends, batch_idx, n_nodes, max_depth_estimate, depth);
    __syncthreads();
...
__global__ void single_source_dep_back_kernel_opt_pure(
        ...
        int n_nodes, int edge_size, int gpu_id, double h2d_time = 0.0)
{
    ...
    // pure版: 上限なしで上書き可 (OOM 実験のため; UM 版との対比確認に使用)
    if (const char *env = getenv("BC_BATCH_OVERRIDE")) {
        int override_val = atoi(env);
        if (override_val > 0) {
            if (override_val > BATCH_PER_STREAM)
                fprintf(stderr, "  > [Warn] BC_BATCH_OVERRIDE=%d exceeds safe limit %d; may cause cudaMalloc OOM\n",
                        override_val, BATCH_PER_STREAM);
    ...
    struct DynBuf {
        ...
        int prev_batch; // 前回このバッファを使ったときのバッチサイズ (6a リセット用)
    };
    DynBuf bufs[NS];
    ...
    for (int i = 0; i < NS; i++) {
        ...
        CUDA_ERR_CHK(cudaMalloc(&bufs[i].d_S_ends,
            (size_t)BATCH_PER_STREAM * (max_depth_estimate + 1) * sizeof(int)));
    ...
    *d_overflow = 0;
    ...
    for (int s_start = 0; s_start < n_nodes; s_start += BATCH_PER_STREAM) {
        ...
        // [最適化6a] 初回はフル memset、再利用時は到達済み頂点のみリセット
        ...
        if (!buf_used[si]) {
            ...
            reset_visited_batch_kernel_pure<<<bufs[si].prev_batch, tpb, 0, st>>>(
        ...
        if (avg_deg < 5.0) {
            nvtxRangePushA("BFS_kernel_shared_frontier_pure");
            const int K          = K_SOURCES_PER_BLOCK;
            const int tpb_shared = K * THREADS_PER_SOURCE; // 256
            int grid_shared      = (curr_batch + K - 1) / K;
            brandes_bfs_kernel_shared_frontier_pure<<<grid_shared, tpb_shared, 0, st>>>(
                ...
                bufs[si].d_depth,  s_start, curr_batch);
            nvtxRangePop(); // BFS_kernel_shared_frontier_pure
        } else {
        ...
        if (avg_deg < 8.0) {
            brandes_back_kernel_tpv_opt_pure<<<curr_batch, tpb, 0, st>>>(
    ...
    fprintf(stderr, "  > [GPU Phase] BFS wall=%.4f s (cum=%.4f s), Backward wall=%.4f s (cum=%.4f s)\n",
...
//  公開エントリポイント: CSR 直受け版
// ============================================================
vector<double> brandes_gpu_opt_pure_csr(const int* R, const int* C,
...
vector<double> brandes_gpu_opt_pure(Graph &G)
{
    ...
    return brandes_gpu_opt_pure_csr(R, C, n_nodes, edge_size);
}
    ...
    const int* R, const int* C, int n_nodes, int edge_size, int source)
{
    ...
    *d_overflow_ss = 0;
    ...
    brandes_bfs_kernel_opt_pure<<<1, tpb>>>(
        d_R, d_C, n_nodes,
    ...
    single_source_dep_back_kernel_opt_pure<<<1, tpb>>>(
