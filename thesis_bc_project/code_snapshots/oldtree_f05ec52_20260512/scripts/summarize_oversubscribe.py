...
デフォルト:
  tsv_path = <script_dir>/../build_miyabi/result_um_oversubscribe/oversubscribe_results.tsv
  log_path = <script_dir>/../build_miyabi/result_um_oversubscribe/um_experiment.log
"""
...
def build_parser() -> argparse.ArgumentParser:
    default_dir = os.path.join(
        ...
        "..",
        "build_miyabi",
        "result_um_oversubscribe",
    )
    parser = argparse.ArgumentParser(description="HBM3 oversubscribe 実験の集計")
    parser.add_argument(
        "tsv_path",
        nargs="?",
        default=os.path.join(default_dir, "oversubscribe_results.tsv"),
        help="oversubscribe_results.tsv のパス",
    )
    parser.add_argument(
        "log_path",
        nargs="?",
        default=os.path.join(default_dir, "um_experiment.log"),
        help="um_experiment.log のパス",
    )
    return parser
