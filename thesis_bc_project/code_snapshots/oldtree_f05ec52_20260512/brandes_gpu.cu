		...
		for (int i = warp_id; i < nodes_in_level; i += num_warps_in_block) {
			...
			for (int j = R[w] + warp.thread_rank(); j < R[w+1]; j += warp.size()) {
				...
				if (d_d[batch_idx * n_nodes + v] == d_d[batch_idx * n_nodes + w] + 1) {
					local_sum += (sigma_w / d_sigma[batch_idx * n_nodes + v]) * (1.0 + d_delta[batch_idx * n_nodes + v]);
				}
			}
...
__global__ void brandes_bfs_kernel(
	...
	if (tid == 0) {
		for (int v = 0; v < n_nodes; v++) {
			...
			d_sigma[batch_idx * n_nodes + v] = (v == s) ? 1.0  :  0.0;
...
__global__ void brandes_kernel_cooperative(int *R, int *C, double *CB, int n_nodes, int *d_d, double *d_sigma, int *d_Q_curr, int *d_Q_next, int *d_S, int *d_S_ends, double *d_delta, int s_start) {
	int batch_idx = blockIdx.x;
	int s = s_start + batch_idx;
	int tid = threadIdx.x;
...
#pragma omp parallel num_threads(num_gpus)
	{
		...
		int *d_R, *d_C, *d_d, *d_Q_curr, *d_Q_next, *d_S, *d_S_ends;
		double *d_sigma;
		double *d_CB, *d_delta;
		...
		const size_t per_batch_mem =
			n_nodes * (3*sizeof(int) + 2*sizeof(double)) +
			n_nodes * sizeof(int) +
		...
		CUDA_ERR_CHK(cudaMalloc(&d_sigma, BATCH_SIZE * n_nodes * sizeof(double)));
