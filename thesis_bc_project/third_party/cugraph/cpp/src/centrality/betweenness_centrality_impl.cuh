    ...
    thrust::for_each(
      ...
      [d_sigma_2d    = sigmas_2d.begin(),
       ...
       v_first = graph_view.local_vertex_partition_range_first()] __device__(auto tagged_source) {
        ...
        auto v_offset = static_cast<size_t>(v - v_first);
        auto idx      = static_cast<size_t>(origin) * local_vertex_partition_range_size + v_offset;
        d_distance_2d[idx] = 0;
        d_sigma_2d[idx]    = 1;
      });
  }
          ...
          cuda::proclaim_return_type<bfs_edge_tuple_t>(
            [d_sigma_2d = sigmas_2d.begin(),
             ...
             v_first = graph_view.local_vertex_partition_range_first()] __device__(auto tagged_src,
              ...
              auto src_offset = static_cast<size_t>(src - v_first);
              auto src_idx =
                static_cast<size_t>(origin) * local_vertex_partition_range_size + src_offset;
          ...
          cuda::proclaim_return_type<bool>(
            [d_distances_2d = distances_2d.begin(),
             ...
             invalid_distance] __device__(auto tagged_src, auto dst, auto, auto, auto) {
              auto origin     = cuda::std::get<1>(tagged_src);
              auto dst_offset = static_cast<size_t>(dst - v_first);
              auto dst_idx =
                static_cast<size_t>(origin) * local_vertex_partition_range_size + dst_offset;
      ...
      thrust::for_each(handle.get_thrust_policy(),
                       ...
                       [d_distances_2d = distances_2d.begin(),
                        ...
                        hop] __device__(auto tuple) {
                         ...
                         auto v_offset = static_cast<size_t>(v - v_first);
                         auto idx =
                           static_cast<size_t>(origin) * local_vertex_partition_range_size + v_offset;
  ...
  bool const do_expensive_check)
{
  ...
  if constexpr (multi_gpu) {
    ...
    if (max_sources_per_batch > 1) {
      // Dynamic batch sizing (mirrors gpu_opt approach):
      // Query actual free GPU memory at call time.
      // With managed_memory_resource, actual usable memory extends to CPU DRAM
      // via NVLink-C2C, but cudaMemGetInfo only reports GPU HBM.
      // Use conservative 20% budget for static per-source arrays to leave
      // ample room for frontier expansion, sort temporaries, and reduce_by_key.
      size_t free_mem_bytes = 0;
      size_t total_mem_bytes = 0;
      RAFT_CUDA_TRY(cudaMemGetInfo(&free_mem_bytes, &total_mem_bytes));
      auto constexpr static_budget_fraction = 0.20;
      auto bytes_per_source = static_cast<size_t>(graph_view.local_vertex_partition_range_size()) *
                              (sizeof(vertex_t) + sizeof(edge_t) + sizeof(weight_t));
      max_sources_per_batch =
        std::max(static_cast<size_t>(free_mem_bytes * static_budget_fraction) / bytes_per_source,
                 size_t{1});
      max_sources_per_batch = std::min(max_sources_per_batch, num_sources);

      // CUGRAPH_BC_MAX_SOURCES_PER_BATCH: hard override (user responsibility for OOM).
      if (auto env_cap = std::getenv("CUGRAPH_BC_MAX_SOURCES_PER_BATCH"); env_cap != nullptr) {
        char* end{};
        auto parsed = std::strtoull(env_cap, &end, 10);
        if ((end != env_cap) && (*end == '\0') && (parsed > 0)) {
          max_sources_per_batch =
            std::max(std::min(static_cast<size_t>(parsed),
                              static_cast<size_t>(std::numeric_limits<uint16_t>::max())),
                     size_t{1});
        }
      }
      std::fprintf(stderr,
                   "  > [cuGraph] free_mem=%.1f GB, max_per_batch=%zu, num_sources=%zu\n",
                   free_mem_bytes / 1e9,
                   max_sources_per_batch,
                   num_sources);
    }
    ...
    std::fprintf(stderr,
                 "  > [cuGraph] num_batches=%zu (max_per_batch=%zu)\n",
                 num_batches,
                 max_sources_per_batch);

    double bfs_total_sec = 0.0, back_total_sec = 0.0;
    for (size_t batch_idx = 0; batch_idx < num_batches; ++batch_idx) {
      ...
      handle.sync_stream();
      auto t_bfs_start = std::chrono::steady_clock::now();
      ...
      handle.sync_stream();
      auto t_back_start = std::chrono::steady_clock::now();
      bfs_total_sec += std::chrono::duration<double>(t_back_start - t_bfs_start).count();
      ...
      handle.sync_stream();
      back_total_sec += std::chrono::duration<double>(std::chrono::steady_clock::now() - t_back_start).count();
    }
    std::fprintf(stderr, "  > [cuGraph Phase] BFS: %.4f sec, Backward: %.4f sec\n",
                 bfs_total_sec, back_total_sec);
  }
