...
Tier A は Sequential CPU 実装を独立参照、GPU_Opt b512 を candidate とする full-vector comparison であり、各グラフ 1 回の vector を比較した。実行識別子は Appendix A の provenance に示す。
...
Tier B は修正版 `325557_3216152_corrected_v1` の 6 vector から構成する。対象は GPU_Opt b1024/b9792、GPU_Opt_Pure b1024、GPU_Opt_Pure_Chunked b1024/b16384、PathMerge b4096 である。測定系列の識別子は Appendix A の provenance に示す。
...
旧 325557 入力は、1-based を 0-based として格納し、隣接要素 7 個の不足、範囲外 ID、頂点行の欠落を含む malformed input であることが integrity audit で判明した。旧入力の保存場所と SHA256 は内部 provenance に保持する。

この入力上の historical canonical series では、same-implementation different-batch の stress 比較が formal tolerance を超え、PathMerge cross でも差が観測され、正式判定は `CORE_FAIL` であった。これらを削除または PASS へ relabel せず、関連する vector、log、SHA256 record とともに historical invalid-input evidence として保持する。

current active conclusion は決定的な修復手続きで再構成した修正版入力を再検証した Tier B である。経緯は「過去の結果を誤りとして削除した」のではなく、「入力不整合を発見し、旧 evidence を保持した上で修正版を再検証した」と記述する。旧 `CORE_FAIL` を current failure として扱わず、修正版 PASS を旧入力の判定へ遡及適用しない。
